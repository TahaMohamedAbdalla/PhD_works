...
#TOPO_FOLDER = BASEDIR + '/ospf6'

def topology_information_extraction():
	
	# Let's parse the input
	period = 5 #float(opts.period)
	routers = []
	ports = '2606'
	port = '2606'
	routers = ['2012::2']
	...
	while (True):
		...
		for router in routers:
			...
			password = PASSWD
			try:
				tn = telnetlib.Telnet(router, 2606) # Init telnet
			except socket.error:
			...
			tn.write(b"show ipv6 ospf6 route detail \r\n")
			# Close
			tn.write(b"q \r\n")
			# Get results
			route_details = str(tn.read_all().decode("utf-8") )
			...
			tn.write(b"terminal length 0 \r\n")
			# Get routing info from ospf6 database
			tn.write(b"show ipv6 ospf6 database detail \r\n")
			# Close
			tn.write(b"q \r\n")
			# Get results
			network_details = str(tn.read_all().decode("utf-8"))
			...
			with open("%s/route-detail-%s-%s.txt" %(TOPO_FOLDER , router, port), "r") as route_file:
				...
				for line in route_file:
					...
					n = re.search('AS-External Id: (\d*.\d*.\d*.\d*) Adv: (\d*.\d*.\d*.\d*)', line)
					if(m or n):
						link_state_id = m.group(1) if m else n.group(1)
						adv_router = m.group(2) if m else n.group(2) 
					    # Get the network id
		...
		for net in list(stub_networks.keys()):
		...
		with open("%s/topo-graph.json" %(TOPO_FOLDER), 'w') as outfile:
...
if __name__ == '__main__':
	...
	#opts = parseOptions()
	# Deploy topology
	topology_information_extraction()
